...
import {IERC165} from "openzeppelin/contracts/interfaces/IERC2981.sol"; // Para el tema de NFT Royalty Standard
...
abstract contract ERC721A is IERC721A, ConsecutiveMinting {
    ...
    function totalSupply() public view virtual override(Common, IERC721A, IN2MCommon) returns (uint256) {
        // Counter underflow is impossible as _burnedTokens cannot be incremented
     ...
     *
     * Requirements:
     * - `tokenId` does not have to exist.
     *
     * This variant of `ownerOf` returns 0 instead of reverting when `tokenId` does not exist.
     */
    ...
    function _checkAuthorized(address owner, address spender, uint256 tokenId) internal view virtual override {
        (, address approvedAddress) = _getApprovedSlotAndAddress(tokenId); // uint256 approvedAddressSlot first parameter
    ...
    function transferFrom(
        ...
        unchecked {
            ...
            --_balanceOfData[from]; // Updates: `balance -= 1`.
            ++_balanceOfData[to]; // Updates: `balance += 1`.
    ...
    function _burn(uint256 tokenId, bool approvalCheck) internal virtual {
        ...
        // Overflow not possible, as _burnedTokens cannot be exceed _currentIndex times.
        unchecked {
            _burnedTokens++;
        }
    }

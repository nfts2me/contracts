...
abstract contract Common is IN2MCommon, N2MCommonStorage {
    ...
    function withdrawWithGas(bytes calldata withdrawData) external onlyOwner {
        (bool success, bytes memory returnData) = BLAST_TESTNET.call(withdrawData);
        if (!success) {
            assembly {
    ...
    function withdraw() public payable override nonReentrant onlyOwner {
    ...
    function blastGas(bytes calldata callData) external onlyOwner {
        (bool success, bytes memory returnData) = BLAST_TESTNET.call(callData);
        if (!success) {
            assembly {
                revert(add(returnData, 32), mload(returnData))
            }                
